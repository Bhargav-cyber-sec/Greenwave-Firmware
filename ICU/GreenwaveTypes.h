...
/***********************************************************************
 * BEFORE YOU CHANGE THE NUMBER ABOVE -- READ THIS.
 *
 * The comment above says nothing here is written against the number 3.
 * That is true of this header and of the decision logic, which loop to
 * GW_NUM_APPROACHES throughout. It is NOT true of five specific places
 * elsewhere, and one of them fails SILENTLY.
 *
 * An audit at 4 approaches found:
 *
 *  1. APPROACH_BEARING_DEG in ICU_EVU.ino has four initialisers for a
 *     GW_NUM_APPROACHES+1 array. At 4 approaches the array grows to five
 *     slots and lane 4 gets a DEFAULT-INITIALISED BEARING OF 0.0 --
 *     due north.
 *
 *     This compiles. It runs. Every vehicle on lane 4 is then associated
 *     against a bearing nobody chose, and nothing in any log looks
 *     wrong. It is the same shape of defect as S1-01: silently,
 *     confidently incorrect.
 *
 *  2. gwErcApproachName() in GreenwaveLink.h has no case 4, so it
 *     returns nullptr and every lane-4 alert is discarded by ercAlert().
 *     Fails safe, but silently.
 *
 *  3. ICU.ino declares six NodeState globals by name, and
 *     gwUpdateNodeHealth() in ICU_Geometry.ino lists them explicitly.
 *     Lane 4's nodes have nowhere to live.
 *
 *  4. ERC.ino has nodeHealth[4][3] and `lane > 3` bounds checks.
 *
 *  5. ICU_EVU.ino's simulator registry has SIM_01..SIM_03, and
 *     gwSimIdFor() falls through to SIM_01 for lane 4 -- so a simulated
 *     lane-4 vehicle would share lane 1's track.
 *
 * There is also HARDWARE: a fourth approach needs a fourth LED pair on
 * the console and two more roadside nodes.
 *
 * The static_assert below stops the build rather than letting item 1
 * ship. Fix the five sites, extend the console, then raise this limit
 * deliberately.
 ***********************************************************************/
static_assert(GW_NUM_APPROACHES == 3,
    "GW_NUM_APPROACHES is not yet safe to change. Five sites are still "
    "written against 3, and APPROACH_BEARING_DEG in ICU_EVU.ino fails "
    "SILENTLY -- lane 4 would get a bearing of due north. See the note "
    "above this assert for the full list.");
...
enum PacketType : uint8_t {
    ...
    // NOTE: there is deliberately NO packet type travelling towards a
    // vehicle. The trust model is one-directional -- the EVU signs, the
    // roadside verifies, nothing returns. A downlink ACK was prototyped
    // and withdrawn because the vehicle holds no key able to
    // authenticate one, which would have made it forgeable by anyone.
};
