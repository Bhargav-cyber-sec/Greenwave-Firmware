...
**Lane 1 (LEFT) · node 1 · FAR (outer — first node an approaching vehicle passes)**
ICU public-key table index **0** = `(lane-1)*2 + (node-1)`

---

## What this folder is

One of six identical sketches. `RDU_L1N1.ino` is **byte-identical** to the
other five — see the root `README.md` §3. Everything that makes this node
different from the others is in `GreenwaveKeys.h`.

**Do not edit `RDU_L1N1.ino` here.** Edit `../RDU_canonical.ino.MASTER`, fan
out to all six, then run `../verify_rdu_tree.py`.

## Keys — **REAL** — verified against ICU table index 0
...
2. **Label the box immediately:** `L1N1 — lane 1 FAR`.
   Six identical boards is exactly how a node ends up on the wrong post.
3. **Read the boot banner.** Confirm `lane=1 node=1`, slot 0, and a config
...
| | |
|---|---|
| `RDU_L1N1.ino` | identical in all six |
...
|---|---|
| 100% `BAD TAG` from this node only | wrong key file — verify against ICU index 0 before reflashing |
