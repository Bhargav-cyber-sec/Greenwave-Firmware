...
const uint8_t ROOT_CA_PUBLIC_KEY[32] = {
     0x41, 0x86, 0x57, 0x62, 0x6d, 0x31, 0x70, 0xca,
     0x93, 0x99, 0x08, 0x38, 0x3d, 0x60, 0xd0, 0xe7,
     0x51, 0x52, 0xe0, 0x8b, 0x52, 0x63, 0xf7, 0xd2,
     0x36, 0x19, 0xb2, 0x51, 0x85, 0xf2, 0x10, 0x08,
};
...
//
// PAIRED WITH HEARTBEAT_TIMEOUT_MS IN ICU.ino (60000, derived from this).
// Change one, recompute the other -- and recompute the SUM, not the term.
//
// v7 CORRECTION. This comment said 40000 and the ICU has been running
// 60000. The ICU value is the correct one and is NOT changed here; only
// this documentation was stale. ICU.ino records the reasoning:
//
//   "Raised 40000 -> 60000 alongside NODE_FAILED_MS in ICU_Geometry.ino.
//    The RDU heartbeat interval doubled to 10 s as part of the
//    duty-cycle fix, so worst-case spacing from a healthy node is now
//    17.5 s. Leaving this at 40 s would have marked nodes offline here
//    while the geometry layer still considered them merely SUSPECT --
//    two subsystems disagreeing about whether the same node is alive."
//
// That reasoning is verifiable from this file: HEARTBEAT_BASE_MS is
// 10000, not the 5000 the derivation block above was written against.
// The 40000 figure predates the duty-cycle change. 60000 is right, and
// it matches NODE_FAILED_MS (60000UL, ICU_Geometry.ino) exactly, which
// is the consistency that mattered.
//
// Recomputed margin at the current constants:
//   10000 + 1500 + 6000 + 130 = 17630 ms worst spacing
//   60000 / 17630 = 3.4 clean misses.
#define HEARTBEAT_MAX_DEFER_MS 6000UL

/***********************************************************************
 * v7 : ICU CONSTANTS THIS NODE IS BUILT AGAINST
 *
 * These are NOT this node's settings. They are the ICU values that this
 * node's own timing was derived from, declared here so the boot banner
 * can print them as constants instead of as string literals.
 *
 * That distinction is the whole point. The banner used to say
 * "(ICU expects hbTimeout=40000)" as a hardcoded literal inside a
 * printf. When the ICU moved to 60000 nothing in this sketch had to
 * change for it to keep compiling, so the literal silently became
 * wrong -- and the banner whose entire job is catching a mismatched
 * flash started asserting one.
 *
 * ICU.ino hit precisely this defect in v3.7 and fixed it the same way:
 * "a banner that lies is worse than no banner, because it converts
 * 'unknown' into 'confidently wrong'."
 *
 * Still not compiler-checkable across two sketch folders. But
 * verify_rdu_tree.py now parses BOTH sides and fails if they disagree,
 * so the pairing is checked by something other than a pair of eyes.
 ***********************************************************************/
#define EXPECTED_ICU_HB_TIMEOUT_MS     60000UL   // ICU.ino HEARTBEAT_TIMEOUT_MS
#define EXPECTED_ICU_EVENT_TIMEOUT_MS  20000UL   // ICU.ino EVENT_TIMEOUT_MS
...
LoRaEventFrame buildLoRaEvent(const char *vehicleID, int rssi, float snr, uint32_t txSeq,
    ...
    // v7: bit 5. Set when the geofence acted as a real admission gate for
    // this packet. PASS without ENFORCED means "the check would have
    // passed it, but nothing was riding on that" -- which is what every
    // frame from this build has silently meant since the bypass went in.
    //
    // Wire-compatible: bit 5 of an existing uint8_t that was previously
    // always zero. An ICU that does not know the bit ignores it and
    // behaves exactly as it does today.
    if (geofenceEnforced)         f.flags |= EVF_GEOFENCE_ENFORCED;
...
void processLoRaPacket(uint8_t *rxBuffer, size_t len, int rssi, float snr, bool isRetroReplay) {
    ...
    // v7: ALWAYS EVALUATE. See the block comment above this function's
    // caller. Previously the bypass short-circuited the call entirely,
    // so the check never ran and its result was reported as "pass"
    // anyway. The ICU was being told the geofence passed about a
    // computation that did not happen.
    //
    // smartGeofenceDecision() is pure apart from its Serial line: three
    // comparisons over a fixed roadPath array. Running it while bypassed
    // costs microseconds and buys the ICU an honest input.
    bool geofenceResult =
        smartGeofenceDecision(vehicleLatitude, vehicleLongitude,
                              vehicleSpeed, vehicleHeading);

    // Was the geofence actually acting as an admission gate for this
    // packet? Two independent ways for the answer to be no:
    //
    //   GEOFENCE_BYPASSED_FOR_SOP   -- deliberately disabled for SOP
    //   !LANE_NODE_POSITION_KNOWN   -- node not surveyed, so the check
    //                                  fails OPEN and its "pass" is
    //                                  vacuous rather than earned
    //
    // Both must be false for this node's geofence verdict to mean
    // anything. Collapsing them into one bit here rather than at the ICU
    // keeps the ICU free of node-configuration knowledge.
    const bool geofenceEnforced =
        (!GEOFENCE_BYPASSED_FOR_SOP) && LANE_NODE_POSITION_KNOWN;

    // ADMISSION IS UNCHANGED. This is the same expression as before,
    // written out rather than folded into the evaluation.
    bool accepted = GEOFENCE_BYPASSED_FOR_SOP ? true : geofenceResult;

    Serial.print("Geofence result: ");
    if (GEOFENCE_BYPASSED_FOR_SOP) {
        // v7: report what the check WOULD have said. A run in which every
        // line reads BYPASSED_FOR_SOP tells you nothing about whether the
        // geofence would have worked; this one does.
        Serial.printf("BYPASSED_FOR_SOP (shadow verdict: %s)\n",
                      geofenceResult ? "would ACCEPT" : "would REJECT");
    } else {
    ...
    // v7: `geofenceResult`, not `accepted`. These were the same variable
    // and are now deliberately different:
    //
    //   accepted        -- "was this packet admitted"   (bypass-dependent)
    //   geofenceResult  -- "would the geofence pass it" (bypass-independent)
    //
    // EVF_GEOFENCE_PASS must carry the second. Carrying the first is what
    // made the flag untrue while the bypass was on.
    LoRaEventFrame event = buildLoRaEvent(vehicleID, rssi, snr, seq, emergency, priority,
                                          gpsValid, securityStatus, timestamp,
                                          geofenceResult, geofenceEnforced);
...
void printAirtimeBudget() {
    ...
    Serial.printf("  Acoustic re-assert  %lu ms  (ICU expects eventTimeout=%lu)\n",
                  ACOUSTIC_REASSERT_MS, EXPECTED_ICU_EVENT_TIMEOUT_MS);
