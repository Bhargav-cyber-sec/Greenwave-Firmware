...
**Lane 3 (RIGHT) · node 1 · FAR (outer — first node an approaching vehicle passes)**
ICU public-key table index **4** = `(lane-1)*2 + (node-1)`

---

## What this folder is

One of six identical sketches. `RDU_L3N1.ino` is **byte-identical** to the
other five — see the root `README.md` §3. Everything that makes this node
different from the others is in `GreenwaveKeys.h`.

**Do not edit `RDU_L3N1.ino` here.** Edit `../RDU_canonical.ino.MASTER`, fan
...
The ICU already holds this node's **public** key at table index 4. The matching
...
2. **Label the box immediately:** `L3N1 — lane 3 FAR`.
   Six identical boards is exactly how a node ends up on the wrong post.
3. **Read the boot banner.** Confirm `lane=3 node=1`, slot 4, and a config
...
| | |
|---|---|
| `RDU_L3N1.ino` | identical in all six |
...
|---|---|
| 100% `BAD TAG` from this node only | wrong key file — verify against ICU index 4 before reflashing |
