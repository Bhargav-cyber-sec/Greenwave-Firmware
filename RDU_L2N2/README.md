...
**Lane 2 (BOTTOM) · node 2 · NEAR (inner — last node before the stop line)**
ICU public-key table index **3** = `(lane-1)*2 + (node-1)`

---

## What this folder is

One of six identical sketches. `RDU_L2N2.ino` is **byte-identical** to the
other five — see the root `README.md` §3. Everything that makes this node
different from the others is in `GreenwaveKeys.h`.

**Do not edit `RDU_L2N2.ino` here.** Edit `../RDU_canonical.ino.MASTER`, fan
...
The ICU already holds this node's **public** key at table index 3. The matching
...
2. **Label the box immediately:** `L2N2 — lane 2 NEAR`.
   Six identical boards is exactly how a node ends up on the wrong post.
3. **Read the boot banner.** Confirm `lane=2 node=2`, slot 3, and a config
...
| | |
|---|---|
| `RDU_L2N2.ino` | identical in all six |
...
|---|---|
| 100% `BAD TAG` from this node only | wrong key file — verify against ICU index 3 before reflashing |
